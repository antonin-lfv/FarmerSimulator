...
def get_available_contracts(db_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT contract_id, description, reward, requirements
            FROM contracts
        ...
        contracts = []
        for row in cursor.fetchall():
            contract_id, description, reward, requirements_json = row
            try:
...
def accept_contract(db_path: str, contract_id: int, worker_id: int) -> bool:
    ...
    try:
        ...
        cursor.execute("""
            SELECT description, reward, requirements FROM contracts WHERE contract_id = ?
        """, (contract_id,))
        ...
        end_time = start_time + game_hours_to_minutes(contract_duration_hours) * 60
        
        # Récupérer le prix de l'ouvrier pour calculer le coût
        ...
        # Créer une action en cours pour le contrat
        cursor.execute("""
...
def complete_contract(db_path: str, ongoing_action_id: int, reward: float):
    """
    Complète un contrat et donne la récompense.
    """
    conn = connect_db(db_path)
    if conn is None:
        return
    ...
    try:
        ...
        # Supprimer l'action en cours
        cursor.execute("DELETE FROM ongoing_actions WHERE ongoing_action_id = ?", (ongoing_action_id,))
        
        conn.commit()
        
    except Error as e:
        print(f"Erreur lors de la completion du contrat: {e}")
        conn.rollback()
    finally:
        conn.close()

def generate_new_contracts(db_path: str):
    """
    Génère de nouveaux contrats aléatoirement.
    """
    conn = connect_db(db_path)
    if conn is None:
        return
    ...
    try:
        ...
        # Vérifier combien de contrats sont disponibles
        cursor.execute("SELECT COUNT(*) FROM contracts")
        current_contracts = cursor.fetchone()[0]
        
        # Générer de nouveaux contrats s'il y en a moins de 3
        if current_contracts < 3:
            contract_templates = [
                ("Livraison de céréales à la coopérative", 300, 800, {}),
                ("Transport de bois vers la scierie", 400, 600, {}),
                ("Nettoyage des parcelles abandonnées", 200, 500, {}),
                ("Aide à la récolte chez un voisin", 350, 750, {}),
                ("Maintenance des équipements agricoles", 250, 450, {}),
                ("Formation en nouvelles techniques", 150, 300, {}),
                ("Surveillance des cultures de nuit", 400, 600, {}),
            ]
            
            contracts_to_add = min(3 - current_contracts, 2)  # Ajouter jusqu'à 2 nouveaux contrats
            
            for _ in range(contracts_to_add):
                template = random.choice(contract_templates)
                description, min_reward, max_reward, requirements = template
                reward = random.uniform(min_reward, max_reward)
                
                cursor.execute("""
                    INSERT INTO contracts (description, reward, requirements)
                    VALUES (?, ?, ?)
                """, (description, reward, json.dumps(requirements)))
        
        conn.commit()
        
    except Error as e:
        print(f"Erreur lors de la génération de nouveaux contrats: {e}")
    finally:
        conn.close()
